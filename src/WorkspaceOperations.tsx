...
const pieColors = ['#75a5ef', '#66c4ac', '#d8b778', '#f08f8f', '#9b8ce3', '#7fc2df', '#c6d77a', '#d38fc7'];
...
export function WorkspaceStatistics({ token, mode }: { token: string; mode: 'analytics' | 'billing' }) {
  ...
  return <>
    ...
    {!data ? <div className="pw-empty" role="status">{error ? 'Statistics unavailable for this selection.' : 'Loading statistics...'}</div> : <>
      <section className="pw-metric-strip" aria-label="Period totals">
        {mode === 'billing' ? <>
          <div><span>Processed/finalized studies</span><strong>{data.totals.processed.toLocaleString()}</strong><small>billing period basis</small></div>
          <div><span>Billing lines</span><strong>{data.total.toLocaleString()}</strong><small>charges and manual records</small></div>
          <div><span>Recorded charges</span><strong className="pw-charge-total">{data.charges.length ? data.charges.map(c => money(c.amountMinor, c.currency)).join(' / ') : '-'}</strong><small>{data.unrecorded} studies without a billing record</small></div>
        </> : <>
          <div><span>Total studies</span><strong>{data.totals.received.toLocaleString()}</strong></div>
          <div><span>Average TAT</span><strong>{duration(data.totals.averageTatSeconds)}</strong><small>{data.totals.tatSampleSize} reported studies</small></div>
          <div><span>Avg TB score</span><strong>{data.totals.averageTbScore == null ? '-' : data.totals.averageTbScore.toLocaleString()}</strong><small>{data.totals.tbScoreSampleSize ?? 0} scored reports</small></div>
      ...
      {mode === 'analytics' && view === 'daily' ? <div className="pw-stats-scroll">
        ...
        <div className="pw-analytics-grid">
          ...
          <section><h2>Study Traffic - 24 Hours</h2><div className="pw-stats-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.hourly} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}><CartesianGrid stroke="#343b46" vertical={false}/><XAxis dataKey="hour" interval={2} tick={{ fill: '#a1aab9', fontSize: 11 }}/><YAxis allowDecimals={false} tick={{ fill: '#a1aab9', fontSize: 11 }}/><Tooltip contentStyle={{ background: '#252b34', border: '1px solid #485465', color: '#edf0f5' }}/><Bar dataKey="studies" fill="#66c4ac" name="Studies" isAnimationActive={false}/></BarChart></ResponsiveContainer></div></section>
          <section><h2>Modality Distribution</h2><div className="pw-stats-chart">{data.modalityDistribution.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Tooltip contentStyle={{ background: '#252b34', border: '1px solid #485465', color: '#edf0f5' }} formatter={(value, name, entry) => [`${value} studies (${entry.payload.percentage}%)`, name]}/><Pie data={data.modalityDistribution} dataKey="count" nameKey="modality" innerRadius="48%" outerRadius="76%" paddingAngle={2} isAnimationActive={false}>{data.modalityDistribution.map((entry, index) => <Cell key={entry.modality} fill={pieColors[index % pieColors.length]}/>)}</Pie></PieChart></ResponsiveContainer> : <div className="pw-empty">No modality data</div>}</div></section>
        </div>
        <table className="pw-data-table pw-daily-table"><thead><tr><th>Date (IST)</th><th>Studies received</th></tr></thead><tbody>{data.daily.map(d => <tr key={d.day}><td>{d.day}</td><td>{d.received}</td></tr>)}</tbody></table>
      </div>
      : <div className="pw-table-scroll"><table className="pw-data-table pw-stats-table"><thead><tr><th>Patient / ID</th><th>Center</th><th>Accession</th><th>Study</th>{mode === 'billing' ? <><th>Processed (IST)</th><th>Units</th><th>Amount</th><th>Billing status</th><th>Invoice</th></> : <><th>Received (IST)</th><th>Reported (IST)</th><th>TAT duration</th><th>TB score</th><th>Replaced</th></>}</tr></thead><tbody>{data.rows.map(row => <tr key={row.id}><td><strong>{row.patient || '-'}</strong><small>{row.patientId || '-'}{row.demo ? ' / Demo' : ''}</small></td><td>{row.center}</td><td>{row.accession || '-'}</td><td>{row.description || row.modality || '-'}</td>{mode === 'billing' ? <><td>{timestamp(row.processed)}</td><td>{row.units ?? '-'}</td><td>{row.amountMinor != null && row.currency ? money(row.amountMinor, row.currency) : '-'}</td><td><span className={`pw-status ${row.billingStatus === 'NOT_RECORDED' ? 'available' : 'reported'}`}>{row.billingStatus?.replaceAll('_', ' ')}</span></td><td>{row.invoice || '-'}</td></> : <><td>{timestamp(row.received)}</td><td>{timestamp(row.reported)}</td><td className="pw-duration">{duration(row.tatSeconds)}</td><td>{row.tbScore ?? '-'}</td><td title={row.replacementHistory || undefined}>{(row.replacementCount ?? 0) > 0 ? `Yes (${row.replacementCount})` : 'No'}</td></>}</tr>)}{!data.rows.length && <tr><td colSpan={mode === 'billing' ? 9 : 9}><div className="pw-empty">No {mode === 'billing' ? 'processed studies' : 'study activity'} in this period</div></td></tr>}</tbody></table></div>}
